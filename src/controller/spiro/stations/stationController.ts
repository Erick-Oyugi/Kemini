...
export const getSwapStations = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    // 1. Extract optional query coordinates from rider app (e.g., ?lat=-1.286389&lng=36.817223)
    const userLat = parseFloat(req.query.lat as string) || -1.286389; // Default Nairobi CBD
    const userLng = parseFloat(req.query.lng as string) || 36.817223;
